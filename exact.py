...
def solve_stowage_3d_min_rehandles_with_cog(
    ...
    y = pulp.LpVariable.dicts("y", (pairs, B, R), lowBound=0, upBound=1, cat="Binary")
    ...
    sum_rehandles = pulp.lpSum(y[(i, j)][b][r_] for (i, j) in pairs for b in B for r_ in R)
    ...
    for i in C: prob += pulp.lpSum(x[b][r][t][i] for b in B for r in R for t in T) == 1, f"place_once_{i}"

    # 2) One container per slot
    for b in B:
        for r in R:
            for t in T:
                prob += pulp.lpSum(x[b][r][t][i] for i in C) <= 1, f"slot_unique_b{b}_r{r}_t{t}"
    ...
    for b in B:
        for r in R:
            for t in T:
                ...
                pulp.lpSum(x[b][r][t-1][i] for i in C), f"no_float_b{b}_r{r}_t{t}"
    ...
    for (i, j) in pairs:
        for b in B:
            for r in R:
                ...
                prob += y[(i, j)][b][r] <= pulp.lpSum(x[b][r][t][i] for t in T), f"r_ub1_i{i}_j{j}_b{b}_r{r}"
                prob += y[(i, j)][b][r] <= pulp.lpSum(x[b][r][t][j] for t in T), f"r_ub2_i{i}_j{j}_b{b}_r{r}"

    bay_moment = pulp.lpSum(weight[i] * b * x[b][r][t][i] for i in C for b in B for r in R for t in T)
    row_moment = pulp.lpSum(weight[i] * r * x[b][r][t][i] for i in C for b in B for r in R for t in T)
    tier_moment = pulp.lpSum(weight[i] * (t - 1) * x[b][r][t][i]for i in C for b in B for r in R for t in T)
