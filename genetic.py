...
def build_ship_from_individual(individual: List[int], containers: List[Dict], num_bays: int, num_rows: int, max_tiers: int):
    ...
    ship = [[[None for _ in range(max_tiers)] for _ in range(num_rows)] for _ in range(num_bays)]
...
def evaluate_stowage(individual, containers, num_bays, num_rows, max_tiers):
    ...
    t_mom = abs(TierMoment(ship)) # We usually want Min Tier Moment (lower CoG), so no Abs() if it is just 'Sum(Weight*Height)'
...
def solve_stowage_genetic(containers: List[Dict], num_bays: int, num_rows: int, max_tiers: int, 
    ...
    stats.register("min", lambda x: tuple(map(min, zip(*x)))) # Min of each objective
